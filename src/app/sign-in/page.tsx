  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (error instanceof Error && error.message === "Failed to bootstrap user profile.") {
        setErrorMessage(
          "Sign-in succeeded, but profile setup failed. Check server env/emulators and try again.",
        );
      } else {
  ...
  return (
    <div className="min-h-screen bg-slate-950 px-6 py-16 text-slate-100 sm:px-10">
      <main className="mx-auto w-full max-w-md rounded-xl border border-slate-700 bg-slate-900/70 p-8">
        <h1 className="text-2xl font-semibold">Sign in to SchoolTrack</h1>
        <p className="mt-2 text-sm text-slate-300">
          Access your platform workspace and module dashboard.
        </p>
        ...
        <p className="mt-6 text-sm text-slate-300">
          New to SchoolTrack?{" "}
          <Link href="/sign-up" className="font-medium text-teal-300 hover:text-teal-200">
            Create an account
          </Link>
        </p>
      </main>
